...
# ---------------------------------------------------------
# MODELO DE RISCO (usado pela API /api/protect/risk-score)
# ---------------------------------------------------------

class RiskModel(BaseModel):
    """Calcula um score de risco determinístico a partir de um payload arbitrário.

    Implementação heurística simples (não é um modelo de ML treinado); serve
    de base para plugar um modelo real mais tarde sem alterar a API pública.
    """

    def __init__(self) -> None:
        super().__init__(name="RiskModel")

    def score(self, payload: Dict[str, Any]) -> float:
        self.validate_payload(payload)
        digest = self.hash(json.dumps(payload, sort_keys=True, ensure_ascii=False))
        # normaliza os primeiros 8 hex chars do hash para um score em [0, 1]
